    ...
    if (nextDataScript) {
      try {
        ...
        for (const product of products) {
          ...
          // Extract original price from Depop data
          const origPriceRaw =
            ...
            null;
          const origPriceNum = origPriceRaw ? parseFloat(origPriceRaw) : null;
    ...
    if (results.length === 0) {
      $('a[href*="/products/"]').each((i, el) => {
        ...
        if (price && image && fullUrl && title) {
          ...
          // Check for struck-through original price in HTML fallback
