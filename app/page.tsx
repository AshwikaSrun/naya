...
import { triggerInstall, isIOSDevice, isStandaloneMode } from '@/components/InstallPrompt';
      ...
      <section className="bg-[#111] px-6 py-24 md:px-10">
        <div className="mx-auto grid max-w-5xl gap-12 md:grid-cols-[1.2fr_0.8fr] md:items-center">
          <div>
            ...
            <p className="mt-5 text-sm font-light leading-relaxed text-white/50">install naya as an app — instant access from your home screen, full-screen experience, and faster loads. no app store needed.</p>
          </div>
          <div className="flex flex-col gap-3">
            <button
              type="button"
              onClick={() => {
                if (!triggerInstall() && isIOSDevice()) {
                  setShowIOSInstall(true);
                }
              }}
              className="rounded-full bg-white px-6 py-3.5 text-[10px] font-medium lowercase tracking-[0.12em] text-black transition-opacity hover:opacity-85"
            >
              install naya
            </button>
            <Link href="/app" className="mt-3 text-center text-[10px] lowercase tracking-[0.15em] text-white/40 transition-colors hover:text-white">
              or try the AI concierge
            </Link>
          </div>
        </div>
      </section>
        ...
        <div className="fixed inset-0 z-[200] flex items-end justify-center bg-black/60 backdrop-blur-sm sm:items-center">
          <div className="mx-4 mb-6 w-full max-w-sm overflow-hidden rounded-2xl bg-white p-8 shadow-2xl sm:mb-0">
            ...
            <div className="mt-6 space-y-4">
              <div className="flex items-start gap-3">
                ...
                <p className="text-sm text-text-secondary">
                  ...
                  <svg className="inline h-4 w-4 align-text-bottom text-blue-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5}>
            ...
            <button
              ...
              className="mt-6 w-full rounded-full bg-black py-3.5 text-[11px] font-medium lowercase tracking-[0.1em] text-white transition-opacity hover:opacity-90"
            >
              got it
            </button>
          </div>
